...
PW=$TIMESKETCH_PW
FILE=$(python3 /opt/lcvr-to-timesketch/python/get_artifact.py $OID "" $PAYLOAD_ID "plaso")
...
# If Plaso file has been downloaded, prepare for ingestion
if [ -f $PARENT_DATA_DIR/$FILE ]; then
    ...
    if [[ $SLACK_NOTIFICATIONS == "yes" ]]; then
    curl -X POST -H 'Content-type: application/json' --data '{"text":"Importing Plaso file into Timesketch--progress will be visible in the Timesketch GUI"}' $SLACK_WEBHOOK_URL
    fi

    # Run timesketch_importer to import Plaso file into Timesketch
